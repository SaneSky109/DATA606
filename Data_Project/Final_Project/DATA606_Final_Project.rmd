...
Cardiovascular disease is a serious problem for many Americans.  According to the CDC, "heart disease is the leading cause of death in the United States" [2]. Though this is a stark statistic, there are steps that can be taken to reduce one's likelihood of contracting cardiovascular disease. A model that can predict heart disease can be leveraged to inform patients on which factors are most correlated with heart disease.
...
After pre-processing the data, there are **68,985 cases** and **11 features**. Each observation represents a patient. The variables are listed in the table below (only showing first 50 observations).

```{r, echo=FALSE}
datatable(cardio.data[1:50,-c(8,13)])
```
...
I also created a model to assess the predicting power with all variables from the dataset in the model, except height and diastolic blood pressure. The model was able to reach an accuracy level of 72.86% in predicting if an unknown patient suffers from cardiovascular disease. This overall performance is not much better than that of the simpler model, therefore the variables in the simple model are the more important variables for predicting cardiovascular disease.
